...
babel.transform(gogJs, {}, (err, result) => {
  ...
  fs.writeFile('./Gog-Game-Library-Check.user.js', userJsText, (error) => { // eslint-disable-line
    if (error) {
  ...
  fs.writeFile('./Gog-Game-Library-Check.all.user.js', userAllText, (error) => { // eslint-disable-line
    if (error) {
      return console.error('Gog-Game-Library-Check.all.user.js文件写入失败: ', error);
    }
    console.log('Gog-Game-Library-Check.all.user.js文件写入成功');
  });
});
