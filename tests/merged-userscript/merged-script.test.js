...
describe('merged userscript contract', () => {
  ...
  describe('raw merged userscript content', () => {
    ...
    it('contains modern minimal ui token markers in raw merged output', () => {
      assert.match(rawMergedText, /rgba\(15,23,42,(?:0)?\.45\)/, 'missing modern mask color token');
      assert.match(rawMergedText, /\.glc-dialog\s*\{[\s\S]*?border\s*:\s*1px solid #e2e8f0\s*;/, 'missing modern dialog border token');
      assert.match(rawMergedText, /\.glc-dialog\s*\{[\s\S]*?border-radius\s*:\s*12px\s*;/, 'missing modern dialog radius token');
      assert.match(rawMergedText, /\.glc-dialog-actions\s*\{[\s\S]*?gap\s*:\s*12px\s*;/, 'missing modern action spacing token');
      assert.match(rawMergedText, /\.glc-toast\s*\{[\s\S]*?border-radius\s*:\s*12px\s*;/, 'missing modern toast radius token');
